...
## v1.5 Stage, Studio, Web (Shipped: 2026-06-12)

**Phases:** 15 (35–49) | **Plans:** 103 | **Tasks:** 205 | **Audit:** tech_debt — 0 unsatisfied requirements, integration CLEAN

**Delivered:** Flow grew from "credible single-author public language" to a real citizen of the
music-software world — pattern matching + Rust-style diagnostics + a pure-Flow test framework,
Tidal-style sequence algebra + generative primitives (Markov/L-system/cellular/chaos) + chord-aware
improv, granular synthesis + time-stretch/pitch-shift + stereo pan, a `live { }` block + modernized
watch + REPL polish + mic input + OSC, notation interop (MusicXML/LilyPond/ABC/MML), real-time MIDI +
clock + JACK transport, module names + strict mode + Beat literals, and — the headline reach track —
a Mono-WASM browser runtime with a real WebAudioBackend behind the greenfield flowlang.dev SvelteKit site.

**Key accomplishments:**

1. **Language foundation (Phase 35):** `(match …)` pattern matching (literal/wildcard/binding/guard + music-aware chord/numeral/articulation extractors), Rust-style multi-line diagnostics with caret spans + Levenshtein "did you mean?", a pure-Flow `(test …)` framework + `flow test` CLI with hermetic isolation, and `-> as name` mid-chain binding.
2. **Sequence algebra & generative (Phase 36):** 13 Tidal-style combinators in `@patterns`, Markov / L-system / cellular-automata / Lorenz-logistic chaos primitives in `@generative` (ref-identity model types), parameterized sections, chord-aware `jam` improv, all determinism-routed through the new `PrngRegistry`.
3. **Sound design & sampler polish (Phase 37):** hand-rolled granular synthesis + phase-vocoder/PSOLA time-stretch + pitch-shift with `#auto` HPS mode dispatch, per-voice constant-power stereo pan + SFZ stereo retrofit, and the sampler-polish bundle (round-robin, velocity crossfade, per-articulation envelopes, warmer piano, sampled drums).
4. **Live coding 2.0 (Phase 38):** `live <quantize> { }` hot-swap block, modernized `flow watch` ANSI status panel, PrettyPrompt-backed REPL (LSP completion, `:help fn`, history search, `(visualize seq)`), mic input via PA_STREAM_RECORD, and an OSC client/server on Rug.Osc.
5. **Notation & studio interop (Phases 39–40):** MusicXML 3.1 + LilyPond export and ABC/MML import (`@notation-io`); real-time MIDI out + 24-PPQN clock master/slave via direct librtmidi P/Invoke (ABI-fixed away from the crashing RtMidi.Core) + best-effort JACK transport. Ableton Link DEFERRED (GPL, D-40-06).
6. **Reach & distribution (Phase 41):** `flow doc` HTML+Markdown generator with in-process example execution, WASAPI + CoreAudio backends, 5-RID self-contained binaries with checksums, JetBrains plugin build/verify, and the EDM third-genre showcase (`examples/edm/pulse.flow`).
7. **Language consolidation (Phases 42–46):** read-only type-system/stdlib audit (`42-AUDIT.md`), `module math` names + qualified imports, `enable strict;` mode, first-class `Nb` Beat literal + `enable beat-true-to-sig;` pragma, and a codebase bloat-removal pass.
8. **WASM runtime & flowlang.dev (Phases 47–49):** `FlowTarget=Desktop|Web` compile-target conditioning → Mono-WASM `flow-lang.dll` with a real JSImport-driven `WebAudioBackend` and frozen `flow-runtime.js` API → a greenfield SvelteKit 2 / Svelte 5 / Tailwind v4 skeuomorphic site (Home/Docs/Playground/Showcase) whose playground runs Flow in the browser. First audible .NET-in-WASM Flow (Firefox-confirmed).

**Verification:** 12/15 phases cleanly `passed`; cross-phase integration CLEAN (7/7 seams wired, E2E flows byte-identical ×2). 0 unsatisfied requirements; 2 by-design defers (MIDI-RT-03 CoreMIDI/WinMM; LINK-01 Ableton Link → v1.6).

**Known deferred at close (composer-deferred HUMAN-UAT, 2026-06-08 — shipped on machine-verified evidence + these as deferred debt):**

- Phase 40 — real hardware synth / DAW master+slave clock / live JACK timebase
- Phase 41 — JetBrains Marketplace publish, osx/win exec smoke, **v1.5.0 GitHub Release cut**
- Phase 48 — Chrome/Safari audible audio (Firefox PASS only)
- Phase 49 — live Cloudflare Pages deploy, GitHub OAuth gist round-trip, cross-browser audio + skeuo visual + screen-reader smoke
- Bookkeeping: Phase 39 has 0 SUMMARY.md (work documented in VERIFICATION); Phases 39/47/48 lack VALIDATION.md (Nyquist partial)
- Full itemized list: STATE.md `## Deferred Items` + `.planning/milestones/v1.5-MILESTONE-AUDIT.md`
