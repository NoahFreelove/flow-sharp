...
stopped_at: "v1.5 Stage, Studio, Web CLOSED 2026-06-12 — 15 phases (35–49), 103 plans archived; milestone audit tech_debt (0 unsatisfied requirements, integration CLEAN); tagged v1.5.0 (local). Deferred human-UAT on Phases 40/41/48/49 + by-design defers MIDI-RT-03/LINK-01 recorded in ## Deferred Items. NEXT: /gsd:new-milestone."
last_updated: "2026-06-12T00:26:17.147Z"
last_activity: 2026-06-12 — Milestone v1.5 completed, archived, and tagged v1.5.0
progress:
  total_phases: 15
  completed_phases: 15
  total_plans: 103
...
See: .planning/PROJECT.md (updated 2026-06-12, after v1.5 close)

**Core value:** Users can write musical ideas as code and hear them immediately -- the language must faithfully translate musical notation into correct, playable audio.
**Current focus:** Planning the next milestone (v1.6). v1.5 shipped + archived + tagged v1.5.0; deferred human-UAT (Phases 40/41/48/49) + by-design defers (MIDI-RT-03, LINK-01) tracked in `## Deferred Items`.

## Current Position

Phase: Milestone v1.5 COMPLETE — archived to `.planning/milestones/v1.5-*`, tagged v1.5.0 (local; not pushed)
Plan: —
Status: milestone_complete — ready for `/gsd:new-milestone`
Last activity: 2026-06-12 — Quick task 260611-x8l: `Note:` now works as a trailing comment (type-annotation-aware so `proc f(Note: x)` still parses). Preceded by 260611-wp2 (note-stream cutoff fix). Both ship in the next WASM bundle regen for the live playground.
...
| 260608-wcy | PolyBLEP-band-limit the saw + square oscillators (v1.6 "Sound Design 2.0", D-37-09) — fixes audible aliasing when bassy saws stack (composer feedback on examples/edm/pulse.flow; confirmed NOT clipping, peak 34% FS). `BlepOscillator.PolyBlep` helper; saw subtracts one residual at the wrap, square does `naive + blep(phase) − blep((phase+0.5)%1)`; absolute-time `(frequency*t)%1.0` phase formula preserved for byte-determinism; sine + triangle untouched. Regen'd Phase46 byte-guard oracle + added Phase29 band-limit richness facts; re-pinned Phase41 showcase baseline (only baseline rendering saw); two-run cmp-clean byte-identical; full suite GREEN (2283 passed; 2 pre-existing WASM console-race flakes pass in isolation). Both Desktop + Web builds 0 errors. | 2026-06-08 | 7e618b8 | [260608-wcy-band-limit-saw-square-oscillators-with-p](./quick/260608-wcy-band-limit-saw-square-oscillators-with-p/) |
| 260608-x94 | iOS-6 skeuomorphic home redesign for flow-site (`/` route ONLY) — rewrote `+page.svelte` from the gitignored design handoff (`index.html`+`flow.css`): linen ground, brushed-aluminum toolbar, Aqua jelly buttons, inset code wells w/ traffic-light dots, stitched-leather audio rack, bottom tab bar, reflection wordmark. Wired the already-ported `highlightFlow` (prerender-time Flow syntax highlighting, pure → `{@html}` precomputed in `<script>`) + `tones` (Web-Audio Play buttons, lazy AudioContext on gesture) helpers; Svelte-5-runes interactivity (`playingIndex` $state + 14-bar reactive VU meters, guarded timers). `+page.ts` collapsed to `export const prerender = true;`; `+layout.svelte` gained `const isHome = $derived(current==='/')` gating the shared `<header>`+mobile-nav behind `{#if !isHome}` so `/docs`//`playground`//`showcase` keep the old Logic-Pro-wood chrome unchanged. 3 hero snippets copied verbatim; light-only (no Toggle on /, no dark rules); frozen `flow-runtime.js` untouched; audio uses tones not WASM. `pnpm -C flow-site build` GREEN (clean prerender of /, prints GUARD_OK); prerendered-artifact inspection confirms iOS-6 markers on / + `site-chrome` preserved on /docs+/showcase + /playground still CSR. Cleanup: dropped 3 unused CSS selectors (.btn.dark/.plate.tight/.code-body). Audio audibility + skeuo visual fidelity = manual/human check per handoff §6. | 2026-06-09 | ca97c5c | [260608-x94-implement-ios-6-home-redesign-for-flow-s](./quick/260608-x94-implement-ios-6-home-redesign-for-flow-s/) |
| 260611-sqk | Remove the iOS-6 bottom tab bar from the flow-site home (`/`) so every route uses ONE top nav (composer: "pick one and stick with it" → top bar). The home shipped a sticky top toolbar AND a fixed bottom `.tabbar` (iOS-6); other routes only the top bar — so the bottom bar vanished on navigation and home showed two bars at once. Removed the `.tabbar` markup+CSS + the footer's 70px bottom reserve; the top pill nav was hidden at ≤600px (the tabbar WAS the mobile nav) so made it visible + horizontally scrollable inside the toolbar (min-width:0/overflow-x:auto, tightened mobile gap) — single top nav works at every width, no 320px overflow. Retargeted 5 tabbar-asserting tests to the single Primary nav. vitest 133/133, build clean, playwright nav/responsive/render-strategy 72/72 (desktop/375/320). | 2026-06-12 | 83eadb5 | [260611-sqk-remove-ios-6-bottom-tab-bar-from-flow-si](./quick/260611-sqk-remove-ios-6-bottom-tab-bar-from-flow-si/) |
| 260611-wp2 | Fix "note streams cut off after ~1 bar" (native + WASM playground). `SequenceData` counted a non-pickup bar as the timesig numerator, but a note stream packs all notes into one bar — a 9-beat 4/4 stream reported 4 beats so MixVoicesToBuffer truncated to one bar (8-note stream → 4 audible; chord stream → 2 chords). New `BarLengthBeats()` helper: monophonic bars use `Math.Max(numerator, actualBeats)` (overfull streams render full; underfull/exact still pad — no layout/determinism regression); parallel `{voice}` bars keep the numerator (GetActualBeats sums simultaneous voices). Regenerated `baselines/Phase37/piano_warmth_smoke.wav` (its 12-beat monophonic streams had been truncated to 4 in the golden file). flow-lang.Tests 2432 passed; remaining Showcase-RMS/ClockSlave/RunFromJs failures pre-existing/flaky (confirmed on clean HEAD). | 2026-06-12 | 102775f | [260611-wp2-fix-note-stream-cutoff-sequencedata-bar-](./quick/260611-wp2-fix-note-stream-cutoff-sequencedata-bar-/) |
| 260611-x8l | `Note:` now works as a TRAILING comment, not just at line start (`(play x) Note: this plays it`). Composer ask from the web player. The naive fix (drop the line-start guard) broke the language: `Note:` is also the proc-parameter TYPE ANNOTATION `Type: name` (`internal proc str (Note: value)`, `proc bumpBeat (Beat: b)`) used throughout the stdlib — commenting those out made std/audio/bars/notation fail to load → core builtins missing. Fix: `IsTypeAnnotationColonPosition()` excludes the one collision (a `Type:` annotation always sits right after `(` or `,`); newline/other preceding content → comment. TODO:/FIXME: unchanged. Lexer/parser suites 277/277; full suite 2428 passed (same flaky/pre-existing Jack/Showcase/Wasm integration failures). | 2026-06-12 | 1b05a65 | [260611-x8l-allow-note-comment-marker-mid-line-trail](./quick/260611-x8l-allow-note-comment-marker-mid-line-trail/) |

## Deferred Items

Items acknowledged and deferred at **v1.5 milestone close on 2026-06-11**. Full audit:
`.planning/v1.5-MILESTONE-AUDIT.md` (status: tech_debt — 0 unsatisfied requirements,
integration CLEAN). Re-surface anytime via `gsd-sdk query audit-open`.

**Composer-deferred HUMAN-UAT** (chosen 2026-06-08; the milestone shipped on machine-verified
evidence + these as deferred debt):

| Category | Item | Status |
|----------|------|--------|
| uat | Phase 40 — real synth / DAW master+slave clock / live JACK timebase (40-HUMAN-UAT.md) | deferred |
| uat | Phase 41 — JetBrains Marketplace publish (JET-01); osx/win exec smoke; v1.5.0 GitHub Release cut (41-HUMAN-UAT.md) | deferred |
| uat | Phase 48 — Chrome + Safari audible audio (Firefox PASS only; 48-HUMAN-UAT.md) | deferred |
| uat | Phase 49 — live CF Pages deploy; GitHub OAuth gist round-trip; cross-browser audio + skeuo visual + SR smoke (49-HUMAN-UAT.md) | deferred |
| verification | Phase 41 — 41-VERIFICATION.md status human_needed (code 7/7 verified; human gates open) | human_needed |
| verification | Phase 49 — 49-VERIFICATION.md status human_needed (20/24 REQ-SITE closed; 4 human) | human_needed |

**By-design scope defers** (documented, no action):
...
| Category | Item | Status |
|----------|------|--------|
| summary | Phase 39 — 5 plans executed, 0 SUMMARY.md (work documented in 39-VERIFICATION.md) | missing-artifact |
| validation | Phase 39 / 47 / 48 — no VALIDATION.md (Nyquist MISSING); Phases 40-46/49 PARTIAL | nyquist-partial |
| debug | open debug sessions: midi-import-quarter-quantize (investigating), midi-voice-block-racing (diagnosed), function-overload-resolution-failures (awaiting_human_verify), wasm-boot-no-app-bundle (fix done, browser-UAT pending) | open |
| quick_task | ~21 quick-tasks without SUMMARY (mostly done work, e.g. repl-fixes/ios6-home/coreaudio); perf bundles A–F + benchmark harness genuinely unstarted | mixed |
| seeds | 3 dormant v1.4-era seeds (LSP / CLI-install / sampler) — addressed by Phases 31/30/33 | dormant |
| test | ~36 pre-existing Phase 28/29/35/38 xUnit failures predate v1.5 base (c4cd738); 0 new introduced by any v1.5 phase | pre-existing |

Known deferred items at close: see audit for the itemized 41-entry `audit-open` snapshot.
