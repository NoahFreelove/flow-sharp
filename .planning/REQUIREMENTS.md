...
- [ ] **REQ-WEB-TARGET-05**: FlowEngine + ExecutionContext + Value + SongRenderer + TestSnapshot `#if !FLOW_WEB` guards close the remaining 13 Web-build compile errors (Sfz / Network references). Target: Plan 47-03.
- [ ] **REQ-WEB-TARGET-06**: ModuleLoader stripped-stdlib gate (`@sfz` / `@osc`) emits charitable `[target] module 'X' unavailable on Web target — line N. Build with FlowTarget=Desktop to enable.` advisory via `WarnOnce`. Parser parse-time gate for `live { ... }` blocks throws Rust-style ParseException when `!FlowEngine.SupportsLiveBlocks`. Target: Plan 47-03.
- [ ] **REQ-WEB-TARGET-07**: DryWetMidi 8.0.3 WASM-compat smoke at Plan 47-04; if it fails to compile under FlowTarget=Web, strip `writeMidi` from Web with parse-error advisory. Target: Plan 47-04.
- [ ] **REQ-WEB-TARGET-08**: `[FlowTargetFact("Desktop", "Web")]` xUnit attribute subclassing `FactAttribute` (sets `Skip` conditionally based on `FLOW_WEB` define matching declared targets); Desktop-only tag sweep across stripped-feature tests. Target: Plan 47-04.
