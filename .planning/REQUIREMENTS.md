...
**Phase 47 SHIPPED 2026-05-25** — `FlowTarget=Desktop|Web` MSBuild conditioning so flow-lang compiles cleanly under WASM by stripping browser-incompatible features (P/Invoke audio backends, FileSystemWatcher, raw UDP sockets, large sample assets). Foundation for Phase 48 (WASM Runtime + WebAudioBackend) + Phase 49 (flowlang.dev site). 6/6 plans complete; 10 REQ-WEB-TARGET-NN closed. Full Desktop test suite preserved (2127 PASS / 9 SKIP / 0 new FAIL); Web build of flow-lang + flow-lang.Tests both clean end-to-end. See `.planning/phases/47-compile-target-flavors/47-VERIFICATION.md` for per-REQ closure evidence + D-47-NN decision trace + 3 known caveats (DryWetMidi WASM-compat verified, test-project Web build closed at Plan 47-06, PATTERNS.md discrepancies acted upon).
...
- [x] **REQ-WEB-TARGET-10**: `AssemblyReferenceScanTests` via Mono.Cecil 0.11.5 (MIT) — reflective scan of Web-compiled `flow-lang.dll` asserts zero references to forbidden type-ref prefixes (Rug.Osc / RtMidi.Core / System.IO.FileSystemWatcher) + zero forbidden P/Invoke substrings (libpulse / AudioToolbox). Paired negative-check sanity Fact (`WebBuild_RetainsLegitimateRefs`) catches over-stripping. Per D-47-14. Mono.Cecil is dev-only test dependency (jbevain canonical maintainer, MIT, 1.4B+ downloads, zero transitive deps); never in flow-lang.dll runtime closure. Shipped Plan 47-05 commits `5c6129c` + `25b40ea`.

### WASM Runtime (Phase 48)

**Phase 48 SHIPPED 2026-06-05** — Mono-WASM publish target ships `flow-lang.dll` into a browser-WASM AppBundle. Real `WebAudioBackend` drives the browser `AudioContext` via `[JSImport]`/`[JSExport]` with constant-power mono→stereo promotion (D-48-07) and a 30s wall-clock cap (D-48-10 — hard on Desktop, best-effort/non-preemptive in single-threaded WASM). The frozen `flow-runtime.js` ES module exposes a 5-export API + 1 convenience to JS consumers (Phase 49 SvelteKit playground tab). `RunResult` shape: structured-error array (D-48-14) + stdout/stderr split (D-48-15) + camelCase JSON via a source-generated `JsonSerializerContext` (trim-safe under TrimMode=full). Bundle size outcome: **3.07 MB compressed Brotli** (canonical Plan 48-05; post-boot-fix Webcil AppBundle re-measures 1.63 MB) vs. the 15 MB target → MONOLITHIC SHIP. DryWetMidi 8.0.3 retained in the Web build (D-48-17 confirmed — `writeMidi` callable from Web → `RunResult.midi`). 3 culture-sensitive call sites fixed for invariant-globalization mode. HUMAN-UAT APPROVED-WITH-FOLLOWUP — **1/3 browser rows signed off PASS (Firefox), 1 deferred (Chrome audio re-smoke), 1 skipped (Safari, no macOS)**. 7/7 plans complete; 19 new xUnit Facts; Phase 47 baseline preserved; zero new NuGet packages. See `.planning/phases/48-wasm-runtime-webaudio-backend/48-VERIFICATION.md` for per-REQ closure evidence + D-48-NN decision trace + 5 known caveats, and `48-PHASE49-HANDOFF.md` for the flow-runtime.js consumption contract.

- [x] **REQ-WASM-BUILD-01**: `dotnet publish flow-lang -p:FlowTarget=Web -c Release` produces a bootable browser-WASM AppBundle; bundle size measured + recorded. Shipped Plan 48-01 commits `74ac158` + `447662f`; library-publish app-bundle generation fixed in Plan 48-06 commit `08140bb` (gated to publish phase by `35dd537`).
- [x] **REQ-WASM-BUILD-02**: `<TrimmerRootDescriptor Include="wasm/trim-roots.xml" />` preserves FlowType subclasses + 21 SpecialTypes + 16 PrimitiveTypes + ArrayType + AudioBuffer + Value + WebAudioBackend (D-48-02; PATTERNS.md Discrepancy 3 — InternalFunctionRegistry NOT enumerated, zero reflection use confirmed by audit). Shipped Plan 48-01 commit `c6dc20d`.
- [x] **REQ-WASM-BUILD-03**: `WasmEnableJiterpreter` + `InvariantGlobalization` + `HybridGlobalization=false` + `TrimMode=full` enabled under `FlowTarget=Web` (D-48-01..03). Shipped Plan 48-01 commit `74ac158`.
- [x] **REQ-WASM-BUILD-04**: `WasmEmitSymbolMap` gated to Debug only — Release strips the symbol map (D-48-04). Shipped Plan 48-01 commit `74ac158`.
- [x] **REQ-WASM-BUILD-05**: 3 culture-sensitive call sites fixed (HarmonyFunctions:441 `ToLower→ToLowerInvariant` + ScaleDatabase:182,233 `ToUpper/ToLower→*Invariant`) for invariant-globalization mode; `CultureInvariantSweepTests` CI gate grep-asserts zero unqualified `.ToUpper()`/`.ToLower()` in production code (D-48-03). Shipped Plan 48-02 commits `8ab1de6` + `a4f726d`.
- [x] **REQ-WASM-DRYWET-01**: DryWetMidi 8.0.3 WASM-compat smoke green via `DryWetMidiWasmPublishTests` — Mono.Cecil scan of the POST-PUBLISH `flow-lang.dll` confirms the `Melanchall.DryWetMidi` reference is retained reachably; `writeMidi` ships on the Web target (D-48-17 confirmed branch; no hand-rolled fallback needed). Shipped Plan 48-02 commit `a4f726d`.
- [x] **REQ-WEBAUDIO-01**: `flow-lang/Audio/FlowRuntimeInterop.cs` with 5 `[JSImport(..., "flow-runtime")]` partial-static methods (createAudioContext / playStereoFloat32 / stopSource / closeContext / resumeContext); module-name string pinned for the JS `setModuleImports('flow-runtime', {...})` wiring (D-48-06). Shipped Plan 48-03 commit `2e15d22`.
- [x] **REQ-WEBAUDIO-02**: `WebAudioBackend` stub-to-real swap — Phase 47 D-47-05 IAudioBackend signatures preserved byte-identical; 7 stub-throw bodies replaced with Mono-WASM impl; Phase 47 `WebAudioBackendStubTests` retired (inverted by the swap). Shipped Plan 48-03 commits `4c7792a` + `028a5e5`.
- [x] **REQ-WEBAUDIO-03**: Constant-power mono→stereo promotion via `PromoteToStereo` before marshal (D-48-07; matches Phase 37 B2 LOCK); 30s wall-clock cap via Task.Run + Wait on Desktop (D-48-10), amended to synchronous execution in single-threaded WASM (Plan 48-06 commits `a8c1911` + `805269c` — Task.Run+Wait deadlocks the one Mono-WASM thread). Shipped Plan 48-03 commit `4c7792a`.
- [x] **REQ-WEBAUDIO-04**: Charitable `IsAvailable()` = `OperatingSystem.IsBrowser()` && JSInterop available, with false-return + log-on-Initialize-failure fallback on non-browser hosts (D-48-11); `Dispose()` never throws. Pinned by `WebAudioBackendIntegrationTests` (8 Facts GREEN on Desktop). Shipped Plan 48-03 commit `4c7792a`.
- [x] **REQ-WASM-API-01**: `flow-lang/wasm/flow-runtime.js` ES module (D-48-12) exports `loadFlowRuntime()` + 4 runtime methods (run / play / stop / dispose) + `resumeAudio()` convenience; frozen 5-export API surface per D-48-13. Shipped Plan 48-04 commit `9afdb26`.
- [x] **REQ-WASM-API-02**: `flow-lang/Runtime/WasmEntry.cs` 4 `[JSExport]` methods (RunFromJs / PlayFromJs / StopFromJs / DisposeFromJs) + `RunResult` + `RunError` POCOs (D-48-14) + stdout/stderr split via per-call StringWriter redirect with finally-restore (D-48-15); source-generated `FlowWasmJsonContext` for trim-safe camelCase + null-omission JSON (Plan 48-06 commit `5b80c01`). Shipped Plan 48-04 commit `dad84b9`.
- [x] **REQ-WASM-API-03**: `flow-lang/wasm/index.html` dev-smoke harness (reference D-48-09 gesture chain) + Web-conditional `CopyToPublishDirectory` for flow-runtime.js + index.html + trim-roots entries preserving WasmEntry/RunResult/RunError. Shipped Plan 48-04 commit `9afdb26`.
- [x] **REQ-WASM-SIZE-01**: Bundle size measured + recorded in `48-BUNDLE-SIZE.md` (self-regenerating via `BundleSizeBudgetTests`); **3.07 MB compressed Brotli** / 10.99 MB uncompressed at the canonical Plan 48-05 measurement (post-boot-fix Webcil re-measure: 1.63 MB / 5.38 MB); hard-asserts < 20 MB cap; D-48-05 decision branch = MONOLITHIC SHIP (lazy-load reserved for v1.6). Shipped Plan 48-05 commit `b2645d5`.
- [x] **REQ-WASM-DET-01**: Two-run cmp-clean determinism preserved via `WasmDeterminismTests` — `RunResult` JSON byte-identical across two runs of the same source after stripping the legitimate `durationMs` wall-clock jitter field (D-48-16). Shipped Plan 48-05 commit `538834c`.

### flowlang.dev Site (Phase 49)

**Phase 49 — EXECUTION COMPLETE, PENDING HUMAN-UAT + LIVE DEPLOY (NOT shipped) 2026-06-05.** All 9/9 plans built + tested in headless CI (vitest 70/70, playwright 275/275, lhci ≥0.9 ×4 both form factors, axe 0-critical), but the phase is **not yet shippable or composer-verified** — THREE human-action gates remain OPEN: (1) live Cloudflare Pages deploy [49-01], (2) GitHub OAuth App + live gist round-trip [49-06], (3) cross-browser AUDIBLE audio + skeuomorphic visual fidelity + screen-reader smoke [49-08]. All three are consolidated into ONE composer pass at `.planning/phases/49-flowlang-dev-site/49-HUMAN-UAT.md`; deploy/OAuth setup is in `49-DEPLOYMENT-RUNBOOK.md`; per-REQ closure + caveats in `49-VERIFICATION.md` (status human_needed). New top-level greenfield `flow-site/` SvelteKit 2 / Svelte 5 / TypeScript / Tailwind v4 project (sibling to flow-lang/ + flow-interpreter/ + flow-cli/), deployed to Cloudflare Pages, consuming the frozen Phase 48 `flow-runtime.js` from its playground tab. Skeuomorphic visual direction (D-49-06, composer-locked). Distribution + reach closer for v1.5. **24 REQ-SITE-*: 20 closed (automated), 4 pending the human gate** (IA-01 deploy portion, DEPLOY-01, PLAYGROUND-03 audible, SHARE-02 live gist). Phase 49 flips to SHIPPED only after the 49-HUMAN-UAT sign-off; Phase 40 + Phase 41 also remain open for v1.5 close.

- [x] **REQ-SITE-IA-01** _(nav portion closed; live-deploy portion PENDING the 49-HUMAN-UAT)_: Five-tab top-level nav (Home / Docs / Playground / Showcase / GitHub) renders on every route; the four local tabs resolve to SvelteKit routes; GitHub is an external `target="_blank"` link with `rel="noopener noreferrer"` + outbound-arrow glyph; `aria-current="page"` on the active route (D-49-07, UI-SPEC §Tabs). Nav E2E green (49-03 `87a9061`). _The site serving live on `<project>.pages.dev` is the deploy half — composer's CF account, runbook §1; flips after prereq A of 49-HUMAN-UAT._
- [x] **REQ-SITE-IA-02**: Per-route render strategy honored — Home / Docs / Showcase prerendered (`export const prerender = true`); Playground is client-only SPA (`ssr = false; csr = true`); `/api/auth/github` is the only server function (D-49-13, RESEARCH Pattern 1). `render-strategy.spec.ts` green (49-03 `7f38308` + 49-07 `e8eea45`).
- [x] **REQ-SITE-IA-03**: SvelteKit 2.x + Svelte 5 + TypeScript + Tailwind v4 (`@theme` CSS-first, no `tailwind.config.js`) + `adapter-cloudflare` scaffold lives under `flow-site/`; `pnpm -C flow-site build` exits 0 producing `.svelte-kit/cloudflare/` output (D-49-11, D-49-13). Build green (49-01 `7bf1295`).
- [ ] **REQ-SITE-DEPLOY-01** _(PENDING the 49-HUMAN-UAT — composer's CF account)_: Cloudflare Pages project created (name `flow-music` else `flow-music-playground` per D-49-36); `<project>.pages.dev` serves the site; `_headers` at project root sets CSP (script-src self + GitHub OAuth domains; `wasm-unsafe-eval` for the runtime) + `Permissions-Policy: microphone=(), camera=(), geolocation=()` + COOP/COEP scoped to `/playground/*` or deferred per RESEARCH Open Q1 (D-49-12, D-49-38). `_headers` built + verified in output (49-01 `98e7ba2`); the live project creation is the composer's CF-account step — runbook §1–§2, flips after prereq A of 49-HUMAN-UAT.
- [x] **REQ-SITE-DESIGN-01**: Design tokens at `flow-site/src/lib/design/tokens.css` + `@theme` mapping — color palette (paper/walnut/brass/slate + dark variants), spacing scale (4/8/12/16/24/32/48/64), type scale (12/14/16/18/24/32/48 with line-height + tracking), shadow scale (1/3/8/16/32), radius (2/4/8/12/24), motion curves (D-49-17, UI-SPEC §Color/Typography/Spacing/Motion).
- [x] **REQ-SITE-DESIGN-02**: Four `.surface-*` material utility classes (`.surface-brushed-metal`, `.surface-wood`, `.surface-paper`, `.surface-felt`) — inline-SVG `feTurbulence` overlays for metal/paper/felt + Sharp-optimized AVIF/WebP/PNG wood-grain raster; each degrades to its flat base under texture-load failure / `prefers-reduced-data` (D-49-18, D-49-32, UI-SPEC §Material Surfaces).
- [x] **REQ-SITE-DESIGN-03**: Skeuomorphic component library at `flow-site/src/lib/components/skeuo/` — exactly 8 base components (Button, Knob, Toggle, Panel, MetalRail, LedIndicator, Slider, Tabs) with the variant/state/a11y/reduced-motion contracts in UI-SPEC §Component Inventory (D-49-19).
- [x] **REQ-SITE-DESIGN-04**: Dark mode is a second skeuomorphic theme (`[data-theme="dark"]` — deep walnut + amber-LED accents), NOT the default; light (paper+walnut+brass) default; toggle in top nav persists in `localStorage`; `prefers-color-scheme` honored first visit (D-49-20).
- [x] **REQ-SITE-DESIGN-05**: A `/design` showcase route renders all 8 skeuo components in light + dark + reduced-motion states for visual-regression baselining (CONTEXT specifics Plan 49-02 acceptance).
- [x] **REQ-SITE-HOME-01**: Home `/` renders the six D-49-21 sections (Hero wordmark + tagline + 3 "Play in playground" example cards + symphony audio embed; value-prop trio; "how it sounds" audio embeds; code-first 20-line annotated snippet; Install + Try-in-browser CTAs; footer) using Plan 49-02 components; all audio embeds gesture-gated, no autoplay (D-49-21, D-49-08, D-49-01).
- [x] **REQ-SITE-DOCS-01**: All 26 wiki markdown pages render at `/docs/[slug]` (lowercase-kebab slug from filename per D-49-27) with shiki-highlighted ```flow code blocks (server-rendered, zero client JS) and an "Open in playground" button per block (D-49-05, D-49-15, D-49-01).
- [x] **REQ-SITE-DOCS-02**: `flow-site/scripts/sync-wiki.sh` clones the wiki (`git clone --depth 1`, `WIKI_REPO_URL` env, fails loudly non-zero) into `src/docs/wiki/` (gitignored), with the in-repo `wiki/` copy as a build seed fallback (D-49-25, RESEARCH Pitfall 6).
- [x] **REQ-SITE-DOCS-03**: `flow-site/src/lib/docs/transform.ts` rewrites GitHub-wiki `[[Page-Name]]` links to `[Page Name](/docs/page-name)`, idempotent, skips fenced code blocks (so `[[1,10],...]` array data in Collections.md is untouched); unit-tested against a synthetic `[[Quick-Start]]` fixture (the real wiki has zero `[[ ]]` links per RESEARCH Pitfall 7) (D-49-26).
- [x] **REQ-SITE-DOCS-04**: `/docs` index renders the wiki TOC grouped into Getting Started / Music Concepts / Audio+Output / Reference from `flow-site/docs-categories.json` (NOT hard-coded), each category a `<Panel header>` of page links (D-49-22).
- [x] **REQ-SITE-PLAYGROUND-01**: `/playground` lazy-imports `/wasm/flow-runtime.js` in `onMount` (committed AppBundle under `static/wasm/`, layout preserved); `loadFlowRuntime()` boots without error; boot failure surfaces in a top-level error pane distinct from per-run errors (D-49-14, D-49-34, HANDOFF §2/§3/§8).
- [x] **REQ-SITE-PLAYGROUND-02**: Monaco editor mounts (browser-only, `?worker` wiring, hand-written Flow Monarch tokenizer derived from `vscode-extension/syntaxes/flow.tmLanguage.json` per RESEARCH Open Q3) in the three-column desktop layout (D-49-23); Run produces `RunResult.stdout` for `(print "hi")` rendered as escaped text (D-49-14, D-49-23, UI-SPEC §Playground).
- [x] **REQ-SITE-PLAYGROUND-03** _(gesture chain + AudioContext-running closed; AUDIBLE confirmation PENDING the 49-HUMAN-UAT)_: Run button onclick calls `runtime.resumeAudio()` AND `runtime.run(source)` in the SAME async frame (D-48-09 gesture chain); `<LedIndicator>` reflects rendering/playing/idle/error via `aria-live`; AudioContext resumes to `running` on a tone snippet (D-49-01, HANDOFF §5). `playground-audio.spec.ts` asserts `state==='running'` (49-05 `46828b4`). _Headless cannot confirm sound is **audible**; Phase 48 left Chrome/Safari audio UNVERIFIED (HANDOFF §7, Firefox PASS only). Per-browser audible sign-off is 49-HUMAN-UAT step 1._
- [x] **REQ-SITE-PLAYGROUND-04**: `RunResult.stdout` / `RunResult.stderr` rendered in a visually-split console sub-region (stdout ink, stderr advisories ink-muted); `RunResult.errors[]` rendered as escaped-text Rust-style diagnostic boxes (no `{@html}`); `RunResult.midi` (Uint8Array) triggers a Blob/anchor MIDI download button (D-48-15, D-48-14, D-48-18, HANDOFF §9, UI-SPEC §Playground error box).
- [x] **REQ-SITE-PLAYGROUND-05**: Monaco is read-only on viewports <768px with a clear banner ("Editing is read-only on small screens…"); Run + audio playback still function for shared snippets (D-49-09, D-49-23, UI-SPEC copywriting).
- [x] **REQ-SITE-SHARE-01**: URL-fragment share is the default path — "Share" copies `/playground#code=BASE64URL(deflate(source))` via fflate; decode is defensive (try/catch → "Couldn't decode this shared snippet", size-cap before loading to guard a decompression bomb); encode↔decode round-trips (unit-tested) (D-49-30, D-49-03 mid-path, Security V5).
- [ ] **REQ-SITE-SHARE-02** _(49-06 CODE COMPLETE — pending the human OAuth-app checkpoint)_: "Save to gist" via GitHub OAuth — CF Worker `flow-site/workers/gist-auth.ts` (≤50 LOC) exchanges the OAuth code for a token server-side (`GITHUB_CLIENT_SECRET` never in client bundle), validates a `crypto.getRandomValues` `state` param (CSRF), redirects only to same-origin `/playground`, requests `scope=gist` (least privilege); client-side POSTs the gist to api.github.com; token in sessionStorage (D-49-28, D-49-29, Security V2). _Worker + client gist POST built + unit-tested with mocked GitHub (`gist-auth.test.ts` 6 green); the live OAuth-App registration + gist round-trip is the Task 4 human-action checkpoint — box flips after that UAT._
- [x] **REQ-SITE-SHOWCASE-01**: `/showcase` gallery (6–12 curated pieces from D-49-24 — symphony, ragtime, third-genre, Markov jazz, OSC live-control, granular showpiece, Carlos Alpha microtonal, Bohlen-Pierce; pieces whose `.flow` source is absent from this worktree link to GitHub source/release assets charitably) + `/showcase/[slug]` detail (gesture-gated hero audio + shiki source block + composer notes + "Open in playground") (D-49-24, D-49-01).
- [x] **REQ-SITE-A11Y-01**: Full keyboard navigation — every interactive element Tab-reachable in visual order, no keyboard traps; Knob/Slider operable with arrow/Home/End/PageUp-Down keys per the `role="slider"` WAI-ARIA contract; brass 2px `:focus-visible` ring + 1px walnut inner edge on every skeuo control (D-49-10, UI-SPEC §Accessibility).
- [x] **REQ-SITE-A11Y-02**: Screen-reader labels on every interactive element (icon buttons carry `aria-label`; Knobs/Sliders labeled with parameter + `aria-valuetext`); `<LedIndicator>` status mirrored in a visually-hidden `aria-live="polite"` region (status not by color alone); ARIA landmarks (`nav`/`main`/`aside`/`footer`); `@axe-core/playwright` reports 0 critical violations on /, /docs, /playground (D-49-10).
- [x] **REQ-SITE-A11Y-03**: `prefers-reduced-motion` honored — knobs become flat sliders, buttons lose travel, cards/panels swap drop shadows for a 1px walnut border, LED pulse becomes steady on/off; flat appearance + correct color/contrast preserved (no information lost) (D-49-10, UI-SPEC §Motion).
- [x] **REQ-SITE-PERF-01**: Lighthouse ≥90 on Performance / Accessibility / Best-Practices / SEO for /, /docs, /playground (mobile + desktop) via `@lhci/cli`; WASM lazy-load keeps the runtime off LCP for marketing/docs routes (D-49-31, D-49-34).
- [x] **REQ-SITE-RESPONSIVE-01**: Layout collapses to a single column <768px (playground stacks controls → editor → console; docs sidebar collapses to a Contents disclosure; nav tabs collapse to a hamburger); showcase audio plays inline on mobile (gesture-gated) (D-49-09, UI-SPEC §Per-Route Layout).
