...
- **D-20 [crossfade-not-in-spec-fixture-but-in-test]:** The Phase 33 CI smoke fixture is a 1-region simple-sustain SFZ; it doesn't exercise the crossfade path. A separate unit test feeds a synthetic 2-region SFZ with `loop_mode=loop_continuous` + `loop_start=22050` + `loop_end=44100`, renders a 4-second sustained note, and runs the per-sample discontinuity check from SPEC acceptance criterion #8.
