...
| `tests/test_*.flow` scripts | `for t in tests/test_*.flow; do dotnet run --project flow-interpreter "$t" > /dev/null 2>&1 \|\| echo "FAIL: $t"; done` | See §Test Gate detail below — Phase 42 closer scoped this gate to "any FAIL line caused by Phase 42 changes is a real regression"; pre-existing fixture failures fall under deferred-items.md and are out of scope per Phase 42's read-only invariant |
