...
**Success Criteria** (what must be TRUE):
  ...
  1. Composer can call `(listMidiPorts)`, `(openMidiOutput "Roland JV-1080")`, and `device.SendNoteOn(channel, pitch, velocity)` / `.SendNoteOff` / `.SendControlChange` / `.SendSysex(data)` against an external hardware synth via the new `IMidiBackend` abstraction (**direct librtmidi P/Invoke** — Plan 40-04 replaced RtMidi.Core 1.0.53 after it proved ABI-incompatible with modern librtmidi ≥4.0). ALSA-seq backend (Linux primary, MIDI-RT-02), CoreMIDI + WinMM backends enabled in Phase 41 cross-platform binary work (MIDI-RT-03). DryWetMidi 8.0.3 remains for offline MIDI file I/O (verified: NO Linux device support upstream — the librtmidi backend is the load-bearing replacement). (MIDI-RT-01, MIDI-RT-02, MIDI-RT-03)
  2. MIDI events emit at `audioBuffer.PlaybackStartTime + bufferOffset` (NOT at queue time) — **best-effort ms-aligned with audio, NOT sample-accurate** (corrected 2026-06-07 per 40-RESEARCH Pitfall 5: the blocking PulseAudio Simple push path has no pull-model callback; `PlaybackStartTime` is the origin seam, MIDI dispatched off a sibling thread). Sysex on separate best-effort queue. Hot-plug failures: log + retry + quiet-drop (NEVER throw — would break long `live` sessions). (MIDI-RT-04 — closed; perceptual "tight enough" check is HUMAN-UAT)
  ...
  4. **LINK-01 DEFERRED to community/v1.6 (resolved 2026-06-07 per D-40-06):** the Phase-40 license review confirmed Ableton Link is GPLv2+/commercial dual-licensed — P/Invoking it from MIT `flow-lang.dll` is a derivative-work contamination hazard (HIGH threat T-40-02). **No Link implementation ships in v1.5** — no `@link` module, no `libabl_link` reference (machine-asserted by `LinkDeferralTests` + `AssemblyReferenceScanTests`). A clean-room / re-licensed binding is welcome as a community PR. **LINK-02 (offline-render determinism) IS shipped + closed independently:** sync tempo is a `play`/`loop`/`preview`-only input, NEVER applied to `writeWav`/`writeMidi`; the `OfflineRenderDeterminismTests` + `LinkDeferralTests` gates pin byte-identical offline render regardless of sync state. (LINK-01 deferred · LINK-02 closed)
...
- [x] 45-01-PLAN.md — Lexer foundation: TokenType.BeatLiteral + signed/unsigned suffix branches + PragmaScanner hyphen gap closure (REQ-BEAT-LEX-01..04, REQ-BEAT-PRAGMA-HYPHEN-01)
...
- [x] 45-05-PLAN.md — (beat N) constructor migration to RegisterContextDependent + DICT-01 regression (REQ-BEAT-CONSTRUCTOR-01..02)
...
- [x] 46-01-PLAN.md — Wave 0 prereqs: D-04 Fixtures verify (already merged) + D-09 diagnostics KEEP rationale + D-03 exact-byte synth guard
- [x] 46-02-PLAN.md — D-02 remove TimelineMap editor-highlighting stack (~250 LOC; zero callers)
- [x] 46-03-PLAN.md — D-05 remove dead internal createSineTone decls + D-08 inline ClampSamples shims
- [x] 46-05-PLAN.md — D-12 Progression DSL unit tests + non-rendered showcase demo + D-16 legacy keep-notes
