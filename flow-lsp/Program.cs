...
//
// Phase 31 plan 31-01 Task 1: extracted top-level statements into an explicit
// public static Main method so the unified `flow` CLI's LspCommand
// (flow-cli/Commands/LspCommand.cs) can delegate via
// `FlowLsp.Program.Main(Array.Empty<string>()).GetAwaiter().GetResult()`.
// Running `dotnet run --project flow-lsp` still works — the runtime calls
// `Program.Main` on the entry assembly the same way.
