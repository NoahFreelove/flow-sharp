...
- **`@midi` surface** (opt-in `use "@midi"`; MIDI-RT-01/02/04 + CLOCK-01/02): `midiPorts()` / `openMidiOutput(port) → MidiDevice` / `midiOut(Song|Sequence, port)` (high-level, GM-routed) + the low-level escape hatch `midiNoteOn`/`midiNoteOff`/`midiCC`/`midiSysex` (for live / `@improv` / generative) + clock `clockMaster(MidiDevice) → ClockHandle` / `clockSlave(String) → ClockHandle` / `clockStop(ClockHandle)`, plus the internal `__enableMidiModule` marker flipping `ExecutionContext.MidiEnabled`. **GM routing (D-40-02):** `midiOut` reuses the Phase 28 `writeMidi` prefix-match table VERBATIM via `InstrumentRouting.ResolveGmProgram` (piano*→0, brass*/horn*→56, sax*→65, flute*→73, string*→48, organ*→19, bell*→14, drum*→ch 10) so a hardware port sounds identical to the exported `.mid`; named-arg `overrides=` Dict (name→channel) layers on top. **Backed by RtMidi.Core 1.0.53** (MIT, .NET Standard 2.0) over ALSA-seq; the clock raw-byte path (0xF8/0xFA/0xFB/0xFC) reaches RtMidi.Core's `internal IRtMidiOutputDevice.SendMessage(byte[])` via reflection (Open Q1, guard-pinned by `RtMidiInternalAccessSpikeTests` — RtMidi.Core PINNED to exactly 1.0.53). Handles are reference-identity Values: `MidiDevice` (specificity 152), `ClockHandle` (153).
...
- **`librtmidi.so` prerequisite.** RtMidi.Core's NuGet ships Win/Mac natives only — Linux needs the system lib: `apt install librtmidi-dev` (pulls `libasound2`). When absent the backend probes + falls back to a silent `NullMidiBackend` (a live session never dies). JACK needs a running server (`jackd`/`qjackctl`); `libjack.so.0` is typically already present.
- **Web-strip (T-40-03).** Every MIDI/JACK file is `#if !FLOW_WEB`-guarded AND `Compile-Remove`'d on Web; RtMidi.Core's PackageReference is under the `!= 'Web'` ItemGroup; `midi.flow` / `jack.flow` are `<None Remove>`'d on Web; `ModuleLoader.IsStrippedOnWeb` rejects `@midi`/`@jack` with a charitable advisory; `AssemblyReferenceScanTests.ForbiddenTypeRefPrefixes` forbids `RtMidi.Core` + `JackSharp`. `dotnet build flow-lang -p:FlowTarget=Web` must stay green (the gate).
