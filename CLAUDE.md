...
### Music-Specific
- **Musical context blocks**: `tempo 120 { }`, `timesig 4/4 { }` (also `timesig C { }` = 4/4 shorthand), `key Cmajor { }`, `swing 0.6 { }`, `voicePool 32 { }` (Phase 28), `tuning t { }` (Phase 32), `octave 3 { }` (quick-260702-gmm — sets the default octave for bare note letters in `| ... |` streams so `octave 3 { | C D E | }` renders C3/D3/E3; applies to main notes, bracket chords `[C E G]`, ghost/grace, and tuplet notes; explicit octave digits like `C5` always win; nesting is innermost-wins; dynamic scope reaches note streams inside called procs/sections; out-of-range N clamps charitably to `[1, 9]` with a one-shot `[octave]` advisory; a bare `Note n = C;` OUTSIDE a stream still defaults to octave 4). All seven keywords (`tempo`/`timesig`/`key`/`swing`/`voicePool`/`tuning`/`octave`) are reserved.
...
- **Multi-track MIDI export** (Phase 28): `writeMidi` emits one track per unique sequence name + conductor track. Prefix-match GM program routing (`piano*`→0, `brass*`→56, `horn*`→60 (French horn; D-16 — precedes brass so horn* doesn't fall through), `sax*`→65, `flute*`→73, `string*`→48, `organ*`→19, `bell*`→14, `drum*`→ch 9/0-indexed percussion). Cross-section same-name sequences concatenate in tick order.
...
- `osc.flow` — OSC client + server (Phase 38 OSC-01/02; opt-in via `use "@osc"`). Rug.Osc 1.2.5 backed (MIT, zero transitive deps). 7 surface builtins + 1 init marker; D-38-13/14/15/16 locked behaviors (charitable type-tag inference / 200 Hz per-path rate limit / bundle depth cap 8 / reference-identity OscHandle). `oscPump` added: handlers queue on the foreground thread; use `(oscPump)` inside a loop to drain. `oscMsg(path, ...args)` (sweep-0614) builds a leaf single-message packet so `(oscBundle (oscMsg ...) ...)` is constructible.
