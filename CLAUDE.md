...
- **Known sampled-instrument quirks (Phase 29 v1.5 follow-ups — STATUS):** the flute's two-sample G4/G5 D5 timbre crossover gap is CLOSED by Phase 37 FLUTE-01 (3rd sample point at A4); sampled-instrument staccato thinness is CLOSED by Phase 37 SAMP-03 per-articulation envelope multipliers (Option A scalar ADSR table at SAMPLE-PATH caller site only — Phase 28 SynthUtils.GenerateArticulationADSR unchanged); sampled drums via SFZ + transient-preserving pitch shift is CLOSED by Phase 37 DRUM-01 (VSCO-CE GM-StylePerc.sfz + PitchShiftEngine.Process #auto routing). Remaining v1.6 backlog: piano EQ shaping curve + sympathetic-string resonance (deferred per D-37-09 narrow-scope, "Sound Design 2.0" if needed); SAMP-03 Option B full per-frame curve overlay (reserved escalation if future UAT iteration flags the staccato gap); sparse-named-arg call ergonomics (OverloadResolver relaxation); Auto-mode HPS rendering cost (current path renders both engines, future v1.6 can render only the chosen frames).
...
- **Granular synthesis primitive (Phase 37 DSP-01):** `(granular buf grain=50ms density=20Hz jitter=0.3 windowing=#hann)` returns a composable Buffer chained-ready for reverb / gain / pan / filter. Windowing options: `#hann` (default), `#gaussian` (σ=0.4 per A2 LOCK), `#tukey` (α=0.5 per A3 LOCK). Jitter PRNG routes through Runtime/PrngRegistry per D-v1.5-06 with distinct generator names `granular_offset` + `granular_timing` (Pitfall 8). Unknown windowing Symbol falls back to Hann + one-shot stderr advisory (Pattern E charitable interpretation). CPU cost proportional to density × grain × sampleRate — composer-trust philosophy, no hard cap. See `examples/dsp/granular.flow` for the runnable tutorial chapter. (Phase 37)
- **Time-stretch + pitch-shift (Phase 37 DSP-02 + DSP-03):** `(stretch buf factor mode=#auto)` time-stretches without pitch change; `(pitchShift buf cents mode=#auto)` shifts pitch without time change. Three modes: `#vocoder` (Laroche-Dolson 1999 phase-locked STFT vocoder for harmonic material), `#psola` (TD-PSOLA + YIN pitch detection for percussive material), `#auto` (Fitzgerald 2010 HPS picks per-frame, emits one-shot stderr advisory `[stretch] mode=#auto picked: X% vocoder / Y% psola across N frames` per D-37-06 + OQ5). 6 W4 LOCK composer knobs thread end-to-end via prefix-ladder arity overloads: `frameSize` (default 2048) / `hopSize` (default 512) / `overlap` (default 4 = CCRMA Hann COLA minimum) / `transientThreshold` (default 0.3 per A1) / `pitchPeriod` (PSOLA YIN override) / `windowSize` (PSOLA grain override). `pitchShift` accepts Double / Cent / Semitone (24 overloads = 3 cents-types × 8 arity steps); 5st = 500c internally. Identity fast-paths (Pitfall 11): factor=1.0 + cents=0 return input byte-identical preserving two-run cmp-clean. Hand-rolled per D-v1.5-03 (RubberBand rejected for GPL hazard); existing `loadWav` varispeed unaffected. See `examples/dsp/stretch_pitchshift.flow` for the runnable tutorial chapter. (Phase 37)
- **Stereo pan retrofit + SFZ polish (Phase 37 MIX-01/02 + SAMP-01/02/03):** Per-voice `Pan` attribute on `Voice` (range -1.0 to +1.0) audit-confirmed shipped at synth path (D-37-15) and pinned via SPEC-8 RMS baseline (`flow-lang.Tests/baselines/Phase37/mix_synth_path_pan.wav`). SFZ retrofit (MIX-02) threads composer voice.Pan via `SfzRenderer.Render` 6-arg overload through `SfzNoteSynthesizer.SectionPan`; effectivePan = clamp(region.Pan + voice.Pan, -1.0, +1.0) per OQ4 additive-with-clamp LOCK. **B2 LOCK (Pitfall 12 resolution):** SFZ-rendered voices ALWAYS promote to stereo via `ToStereoBufferWithPan` — `voice.Pan=0` is "center", not "unset" (there's no PanExplicit flag on Voice); centered effectivePan produces equal L/R at √0.5 via constant-power. SFZ opcode whitelist grows 14 → 20: `seq_position` / `seq_length` for round-robin (per Pattern 5; counter advances modulo seq_length, deterministic via fresh-per-render `SfzRenderer` construction + `ResetAtRenderBoundary`; `seq_length > 100` clamps + WarnOnce per T-37-03-01) + `xfin_lovel` / `xfin_hivel` / `xfout_lovel` / `xfout_hivel` for equal-power velocity-layer crossfade (per Pattern 6 sin/cos curve; 0.7071 sibling-in-band headroom per Pitfall 7; hard-switch fallback preserves Phase 33 byte-identical when sentinels are -1). Per-articulation envelope multiplier table (SAMP-03, A8 Option A LOCK: Staccato (0.5, 1.2, 1.0, 0.8) brightens decay; Marcato/Tenuto/Legato/Accent/Sforzando/Normal distinct scalars) stacks multiplicatively on Phase 28's locked envelope at the SAMPLE-PATH caller site only (Pitfall 10 — Phase 28 `SynthUtils.GenerateArticulationADSR` unchanged; synth-path RMS regression preserved). (Phase 37)
- **Piano warmth (Phase 37 PIANO-01):** Piano SampleCache expands from 2 layers (pp/ff) to 4 layers (pp/mp/mf/ff) at 5 pitch points — 15 disk-loaded + 5 synthesized mp = 20 layers in memory. mp synthesized via signed-RMS interpolation `mp[n] = sign(heavier) × sqrt(pp[n]² × (1-α) + mf[n]² × α)` with α=0.6 (A5 LOCKED, mf-leaning); deterministic + preserves two-run cmp-clean. U-Iowa MIS source ships pp/mf/ff only — mp synthesized at SampleCache eager-load (Pattern 9 Path 1); `RmsInterpolateTruncated` tolerates length-mismatched pp/mf pairs. New `release=` named arg on `renderSong` (Second-typed) exposes sustain-pedal-sim tail length (default 1.5s per Lehtonen 2007 / D-37-11; clamped to [0.05, 10.0] per T-37-04-04 DoS guard); decay time-constant scales with release (releaseSec × 0.3 per RESEARCH §Pattern 8). Threaded via `PianoSynthesizer.CurrentReleaseSec` AsyncLocal<double?> + `SongRenderer.RenderSongWithRelease` (matches Phase 28 `VoiceAllocator._lastPoolSizeUsedForTests` precedent). 4-way crossfade is 3-band delegation to existing 2-way `LoudnessNormalizedCrossfade` (charitable fallback to 2-way pp/ff if mp/mf absent per T-37-04-02). Composer UAT auto-approved per D-37-12 (`37-HUMAN-UAT.md`). (Phase 37)
- **Flute D5 crossover (Phase 37 FLUTE-01):** Flute sample manifest grows G4 / G5 → G4 / A4 / G5; A4 (MIDI 69) chosen over D5 per RESEARCH §Pattern 10 + A6 — closes the low-register varispeed coverage gap (D5 now routes through A4 with 5-semitone upward stretch instead of G4 with 7-semitone, preserving formant content better). Composer drop variant-matched Flute.vib.ff.A4 (vibrato fortissimo) to existing G4/G5 variant. (Phase 37)
- **Sampled drums (Phase 37 DRUM-01):** VSCO Community Edition `GM-StylePerc.sfz` accessible via Phase 33 SFZ surface; `sfz.flow` GM dict grows 19 → 20 entries with `#drums "GM-StylePerc.sfz"` per D-37-13. **W7 LOCK:** the `#drums` dict-symbol is the SOURCE OF TRUTH for percussion detection — `SfzData.IsPercussion` is set at `SfzBuiltins.LoadSfzSymbol` LOAD TIME when `symbolName == "drums"`, NOT by inspecting the filename. The String-overload (`loadSfz "/path/to/X.sfz"`) bypasses dict resolution and inherits `IsPercussion=false` default (composers opt OUT of percussion routing by construction). Drum pitch-shift routes through Plan 37-02's `PitchShiftEngine.Process(raw, semitones*100, StretchMode.Auto)` per D-37-14 — transient-preserving for off-center notes; sample-center notes route through existing Phase 33 varispeed (byte-identical at shift=0). Shifts > 12 semitones emit one-shot stderr advisory per OQ3. (Phase 37)
- **Notation IO surface** (Phase 39 XML-01/02 + LILY-01 + ABC-01/02 + MML-01, opt-in via `use "@notation-io"`): `(writeMusicXML "file.musicxml" song)` emits MusicXML 3.1 partwise (MuseScore-compatible per D-v1.5-08 articulation table — Accent→`<accent/>`, Marcato→`<strong-accent/>`, Staccato→`<staccato/>`, Tenuto→`<tenuto/>`, Sforzando→`<dynamics><sfz/></dynamics>`, Legato→`<slur>` span grouping per D-39-07; multi-track Song → multi-`<part>` per D-39-09; voice blocks → per-note `<voice>N</voice>` tagging; microtonal pitches as decimal `<alter>` cent precision per D-39-06; hand-rolled `XmlWriter` with `NewLineChars = "\n"` for two-run cmp-clean determinism per Pitfall 6). `(writeLilyPond "file.ly" song)` emits LilyPond 2.24+ text (per-Sequence `\new Staff` per D-39-13; voice blocks → `\new Voice` siblings inside `<< { } \\ { } >>`; microtonal as `% +Nc` comments per D-39-12; Dutch pitch convention `cis`/`bes`/etc. per Pitfall 2; `\layout { }` + `\midi { }` blocks kept). `(abc "X:1\nT:Tune\nM:4/4\nK:Cmaj\nC D E F |")` imports ABC 2.1 subset + abc2midi extensions (modal keys Edor/Dmix/Aphr/Cmix/Glyd/Bphr/Floc per D-39-15; `Q:` tempo handles bare BPM + 1/4=BPM + "Allegro" 1/4=BPM forms; multi-tune `X:1`/`X:2` files return `Array[Section]` per D-39-16; charitable per D-39-17 — unknown ornaments dropped with `[abc]` stderr advisory). `(mml "T120 L4 O4 cdefga>c")` imports PC-98 MML common core (notes/accidentals/octave/length/tempo/loops with depth-cap 16 per D-39-19 mirroring T-36-17 DoS guard; FM operator routing + drum-bank opcodes ignored with `[mml]` advisory per D-39-18). XML-02 round-trip CI gate (`mscore --convert-to mxl`) charitable-skips when binary absent per D-39-08. `flow-lang/Vendor/README.md` documents the Plan 39-01/03 decisions to NOT vendor `sightreader/musicxml-schemas` or `matthewcpp/ABCSharp` (XDocument structural diff + hand-rolled ABC parser fit Flow's narrow needs better than ingesting third-party deps; both are MIT-licensed if future v1.6 needs reconsider). (Phase 39)
...
`reverb`, `lowpass`, `highpass`, `bandpass`, `compress`, `delay`, `gain` (dB), `volume` (linear). Phase 37 adds: `granular(buf, grain, density, jitter[, windowing])` (DSP-01); `stretch(buf, factor[, mode, frameSize, hopSize, overlap, transientThreshold, pitchPeriod, windowSize])` (DSP-02); `pitchShift(buf, cents[, mode, ...knobs])` accepting Double / Cent / Semitone cents-arg types (DSP-03). All ride `WindowFunctions` (Hann/Gaussian/Tukey) / `Fft` (radix-2 Cooley-Tukey) / `Hps` (Fitzgerald 2010) shared utilities + `PhaseVocoder` (Laroche-Dolson 1999) / `Psola` (TD-PSOLA + YIN) under `Audio/DSP/`.
...
- `audio.flow` — buffer creation, signal generation, effects, playback convenience functions; Phase 37 adds granular (3 overloads) + stretch (8-arity prefix-ladder) + pitchShift (24 overloads = 3 cents-types × 8 arity steps) forward decls + renderSong(Song, String, Second) for PIANO-01 release= knob
...
- `sfz.flow` — SFZ orchestral sampler surface (Phase 33; opt-in via `use "@sfz"`); Phase 37 DRUM-01 grows the GM dict from 19 → 20 entries with `#drums "GM-StylePerc.sfz"` (W7 LOCK — dict-symbol drives `SfzData.IsPercussion`, NOT filename)
