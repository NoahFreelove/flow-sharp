...
public class DrumSynthesizer : INoteSynthesizer
{
    public AudioBuffer RenderNote(MusicalNoteData note, int sampleRate, double durationBeats, double bpm)
    {
        ...
        {
            36 => RenderKick(sampleRate),             // C2 — Kick
            37 => RenderRimshot(sampleRate),           // C2+ — Rimshot
            38 => RenderSnare(sampleRate),             // D2 — Snare
            42 => RenderClosedHiHat(sampleRate),       // F2+ — Closed HH
            44 => RenderOpenHiHat(sampleRate),         // G2+ — Open HH
            45 => RenderTom(sampleRate, 100.0),        // A2 — Low Tom
            48 => RenderTom(sampleRate, 150.0),        // C3 — Mid Tom
            50 => RenderTom(sampleRate, 200.0),        // D3 — High Tom
            _ => RenderDefaultTick(sampleRate),        // Unmapped — short noise tick
        };
    ...
    private static float[] RenderKick(int sr)
    {
        ...
        for (int i = 0; i < frames; i++)
        {
            ...
            buf[i] = (float)(0.5 * Math.Sin(phase));
    ...
    private static float[] RenderSnare(int sr)
    {
        // Sine 200Hz + white noise, ADSR 1ms/120ms/0/50ms, LP 8kHz
        int frames = (int)(0.171 * sr);
        var buf = new float[frames];

        SynthUtils.GenerateSine(buf, 200.0, 0.3, sr);
        SynthUtils.GenerateWhiteNoise(buf, 0.25);
    ...
    private static float[] RenderClosedHiHat(int sr)
    {
        // White noise, ADSR 1ms/40ms/0/20ms, LP 10kHz
        int frames = (int)(0.061 * sr);
        var buf = new float[frames];

        SynthUtils.GenerateWhiteNoise(buf, 0.2);
    ...
    private static float[] RenderOpenHiHat(int sr)
    {
    ...
    private static float[] RenderTom(int sr, double baseFreq)
    {
        ...
        for (int i = 0; i < frames; i++)
        {
            ...
            buf[i] = (float)(0.4 * Math.Sin(phase));
            phase += 2.0 * Math.PI * freq / sr;
        }
    ...
    private static float[] RenderRimshot(int sr)
    {
        // Noise + sine 500Hz, ADSR 1ms/30ms/0/10ms
        int frames = (int)(0.041 * sr);
        var buf = new float[frames];

        SynthUtils.GenerateSine(buf, 500.0, 0.3, sr);
        SynthUtils.GenerateWhiteNoise(buf, 0.2);
    ...
    private static float[] RenderDefaultTick(int sr)
    {
        // Short noise tick for unmapped notes
        int frames = (int)(0.02 * sr);
        var buf = new float[frames];

        SynthUtils.GenerateWhiteNoise(buf, 0.15);
